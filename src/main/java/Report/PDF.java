...
public class PDF {
	...
	public PDF() {
		try {
			...
			fontTitle = new Font(
					BaseFont.createFont("C:\\Windows\\Fonts\\Arial.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 25,
					Font.NORMAL);
			// Tạo font cho các mục nhỏ bên trong
			fontHeader = new Font(
					BaseFont.createFont("C:\\Windows\\Fonts\\Arial.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 13,
					Font.NORMAL);
	...
	public void writeHoaDon(String mahd) {
		...
		try {
			...
			// Hiện thông tin của hóa đơn hiện tại
			...
			// Khoảng trống giữa hàng
			Chunk glue = new Chunk(new VerticalPositionMark());
			Paragraph paraMaHoaDon = new Paragraph(
					new Phrase("Hóa đơn: " + String.valueOf(hd.getIDHoaDon()), fontData));

			Paragraph para1 = new Paragraph();
			para1.setFont(fontData);
			para1.add(String.valueOf("Khách hàng: " + khBUS.getKhachHangDTO(hd.getIDKhachHang()).getTenKhachHang()
					+ "  -  " + hd.getIDKhachHang()));
			para1.add(glue);
			...
			para2.add(String.valueOf("Nhân viên: " + nvBUS.getNhanVienDTO(hd.getIDNhanVien()).getTenNhanVien() + "  -  "
					+ hd.getIDNhanVien()));
			para2.add(glue);
			...
			Paragraph paraTongThanhTien = new Paragraph(
					new Phrase("Tổng thành tiền: " + PriceFormatter.format(tongThanhTien), fontData));
			paraTongThanhTien.setIndentationLeft(300);
			document.add(paraTongThanhTien);
			Paragraph paraTongKhuyenMai = new Paragraph(
					new Phrase("Tổng khuyến mãi: " + PriceFormatter.format1(tongKhuyenMai), fontData));
			paraTongKhuyenMai.setIndentationLeft(300);
			document.add(paraTongKhuyenMai);
			Paragraph paraTongThanhToan = new Paragraph(
					new Phrase("Tổng thanh toán: " + PriceFormatter.format1(hd.getTongTien()), fontData));
	...
	public void writeHoaDonNhap(String mahdn) throws Exception {
		...
		try {
			...
			Paragraph paraMaHoaDonNhap = new Paragraph(
					new Phrase("Hóa đơn nhập: " + String.valueOf(hdn.getIDHoaDonNhap()), fontData));

			Paragraph para1 = new Paragraph();
			para1.setFont(fontData);
			para1.add(
					String.valueOf("Nhà cung cấp: " + nccBUS.getNhaCungCapDTO(hdn.getIDNhaCungCap()).getTenNhaCungCap()
							+ "  -  " + hdn.getIDNhaCungCap()));
			para1.add(glue);
			...
			para2.setPaddingTop(30);
			para2.setFont(fontData);
			para2.add(String.valueOf("Nhân viên: " + nvBUS.getNhanVienDTO(hdn.getIDNhanVien()).getTenNhanVien()
					+ "  -  " + hdn.getIDNhanVien()));
			para2.add(glue);
//            para2.add("Giờ lập: " + String.valueOf(hd.getGioLap()));
			...
			Paragraph paraTongThanhToan = new Paragraph(
					new Phrase("Tổng thanh toán: " + PriceFormatter.format1(hdn.getTongTien()), fontData));
			paraTongThanhToan.setIndentationLeft(300);
			document.add(paraTongThanhToan);
			document.close();
