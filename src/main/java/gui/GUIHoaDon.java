...
public class GUIHoaDon extends GUIFormContent {
	// Tạo mảng tiêu đề
	private static String array_HoaDon[] = { "Mã hóa đơn", "Mã nhân viên", "Mã khách hàng", "Mã khuyến mãi", "Ngày lập",
			"Tiền giảm giá", "Tổng tiền" };
		...
		inPDF.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent evt) {
				...
				if (i == -1) {
					...
					new PDF().writeHoaDon(String.valueOf(table_HoaDon.tbModel.getValueAt(i, 0)));
				}
			}
		});
		CongCu.add(inPDF);
	...
	public void docDB() {
		...
		if (HoaDonBUS.HD == null) {
			...
			try {
				...
				java.util.logging.Logger.getLogger(GUIHoaDon.class.getName()).log(java.util.logging.Level.SEVERE, null,
						ex);
			}
	...
	protected JPanel TimKiem() {
		...
		// Nút làm mới các field và reset lại table
		...
		ChiTiet.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent evt) {
				...
				try {
					...
					java.util.logging.Logger.getLogger(GUIBanHang.class.getName()).log(java.util.logging.Level.SEVERE,
							null, ex);
				}
				a.setVisible(true);
	...
	public void txtSearchOnChange() {
		...
		setDataToTable(Tool.searchHD(Ten.getText(), Tu_NgayLap.getText(), Den_NgayLap.getText(), donGia1, donGia2,
				cbSearch.getSelectedItem().toString()), table_HoaDon); // chưa sửa xong hỏi Nguyên cái Textfield
	}
