...
public class GUIHoaDonNhap extends GUIFormContent {
	private static String array_HoaDonNhap[] = { "Mã hóa đơn", "Mã nhân viên", "Mã nhà cung cấp", "Ngày nhập",
			"Tổng tiền" };
		...
		inPDF.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent evt) {
				...
				if (i == -1) {
					...
					try {
						new PDF().writeHoaDonNhap(String
								.valueOf(table_HoaDonNhap.tbModel.getValueAt(table_HoaDonNhap.tb.getSelectedRow(), 0)));
					} catch (Exception ex) {
	...
	public void txtSearchOnChange() {
		...
		setDataToTable(Tool.searchHDN(Ten.getText(), Tu_NgayNhap.getText(), Den_NgayNhap.getText(), donGia1, donGia2,
				cbSearch.getSelectedItem().toString()), table_HoaDonNhap); // chưa sửa xong hỏi Nguyên cái Textfield
	}
