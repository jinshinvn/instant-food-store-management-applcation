...
public class GUIBanHang extends GUIFormBanNhap {
	...
	private void Them_click(MouseEvent e) {
		...
		if (i == -1) {
			...
			if (a > SlTrongTable) {
				...
				ThanhToan.addRow(new String[] { String.valueOf(table_MonAn.tbModel.getValueAt(i, 0)),
						String.valueOf(table_MonAn.tbModel.getValueAt(i, 1)),
						String.valueOf(table_MonAn.tbModel.getValueAt(i, 3)),
						String.valueOf(table_MonAn.tbModel.getValueAt(i, 5)), String.valueOf(a) });
				TinhTien();
	...
	private void ThanhToan_click(MouseEvent e) {
		// Ràng buộc dữ liệu
		if (checkText(MaHD.getText(), TongTien.getText(), KhachHang.getText(), NgayLap.getText(), NhanVien.getText(),
				KhuyenMai.getText(), ThanhToan.tb.getRowCount())) {
			...
			HoaDonDTO hdDTO = new HoaDonDTO(MaHD.getText(), NhanVien.getText(), KhachHang.getText(),
					KhuyenMai.getText(), LocalDate.parse(NgayLap.getText()), TienKhuyenMai,
					Float.parseFloat(TongTien.getText()), "Hiện");
			// Thêm vào hóa đơn
			hdbus.them(hdDTO);
	...
	public boolean checkText(String checkMaHD, String checkTien, String checkMaKH, String checkNgay, String checkMaNV,
			String checkMaKM, int somonan) {
		UIManager.put("OptionPane.messageFont", new FontUIResource(new Font("Segoe UI", 0, 20)));
		if (checkMaHD.equals("") || checkTien.equals("") || checkMaKH.equals("") || checkNgay.equals("")
				|| checkMaNV.equals("") || checkMaKM.equals("")) {
