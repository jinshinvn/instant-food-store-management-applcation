...
//        panel tìm kiếm theo thời gian
	...
	private void soLuongNguyenLieuNhap() {
		tb.clear();
		tb.setHeaders(new String[] { "Mã nguyên liệu", "Tên nguyên liệu", "Mã phiếu nhập", "Tên nhà cung cấp",
				"Ngày nhập", "Số lượng", "Ðơn giá", "Tổng chi" });
		...
		for (NguyenLieuDTO nl : qlnlBUS.getNguyenLieuDTO()) {
			...
			tb.addRow(new String[] { nl.getIDNguyenLieu(), nl.getTenNguyenLieu(), "", "", "", "",
					PriceFormatter.format((float) nl.getDonGia()), "" });
			...
			for (HoaDonNhapDTO hdn : qlhdnBUS.search("Tất cả", "", mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) {
				...
				if (cthdn != null) {
					tb.addRow(new String[] { "", "", cthdn.getIDHoaDonNhap(),
							qlnccBUS.getNhaCungCapDTO(hdn.getIDNhaCungCap()).getTenNhaCungCap(),
							String.valueOf(hdn.getNgayNhap()), String.valueOf(cthdn.getSoLuong() + " phần "), "",
			...
			tb.addRow(new String[] { "", "", "", "", mcd.getKhoangTG(), String.valueOf(tongSoLuong) + " phần ", "",
					PriceFormatter.format(tongtiennguyenlieu) });
	...
	private void soLuongMonAnBan() {
		tb.clear();
		tb.setHeaders(new String[] { "Mã món ăn", "Tên món ăn", "Mã hóa don", "Tên nhân viên", "Ngày lập", "Số lượng",
				"Ðơn giá", "Tổng thu" });
		...
		for (MonAnDTO ma : qlmaBUS.getMonAnDTO()) {
			...
			tb.addRow(new String[] { ma.getIDMonAn(), ma.getTenMonAn(), "", "", "", "",
					PriceFormatter.format(ma.getDonGia()), "" });
			...
			for (HoaDonDTO hd : qlhdBUS.search("Tất cả", "", mcd.getNgayTu(), mcd.getNgayTu(), -1, -1)) {
				...
				if (cthd != null) {
					tb.addRow(new String[] { "", "", hd.getIDHoaDon(),
							qlnvBUS.getNhanVienDTO(hd.getIDNhanVien()).getTenNhanVien(),
							String.valueOf(hd.getNgayLap()), String.valueOf(cthd.getSoLuong() + " phần "), "",
			...
			tb.addRow(new String[] { "", "", "", "", mcd.getKhoangTG(), String.valueOf(tongSoLuong) + " phần ", "",
					PriceFormatter.format(tongTienHoaDonTungSanPham) });
			tb.addRow(new String[] { "", "", "", "", "", "" });
			tongTatCa += tongSoLuong;
...
class ThongKeNhanVien extends JPanel {
	...
	public void MonAnCuaTungNhanVien_searchOnChange() {
		tb.setHeaders(new String[] { "Mã nhân viên", "Tên nhân viên", "Mã hóa đơn", "Ngày lập", "Mã món ăn",
				"Tên món ăn", "Số lượng " });
		tb.clear();
		...
		for (NhanVienDTO nv : qlnvBUS.getNhanVienDTO()) {
			...
			for (HoaDonDTO hd : qlhdBUS.search("Mã nhân viên", nv.getIDNhanVien(), mcd.getNgayTu(), mcd.getNgayDen(),
					-1, -1)) { // tương đối -> sai
				tb.addRow(new String[] { "", "", hd.getIDHoaDon(), String.valueOf(hd.getNgayLap()), "", "", "" });

				for (ChiTietHoaDonDTO cthd : qlcthdBUS.search("Mã hóa đơn", hd.getIDHoaDon(), -1, -1, -1, -1)) { // tương
																													// đối
																													// ->
																													// sai
					tongSoLuong += cthd.getSoLuong();
					tb.addRow(new String[] { "", "", "", "", cthd.getIDMonAn(),
							qlmaBUS.getMonAnDTO(cthd.getIDMonAn()).getTenMonAn(),
							String.valueOf(cthd.getSoLuong()) + " phần" });
				}
			}
			tb.addRow(new String[] { "", "", "", mcd.getKhoangTG(), "", "Tổng số món ăn",
					String.valueOf(tongSoLuong) + " phần" });
			tb.addRow(new String[] { "", "", "", "", "", "", "" });
...
class ThongKeKhachHang extends JPanel {
	...
	public void tongTienTungKhachHang_searchOnChange() {
		tb.setHeaders(
				new String[] { "Mã khách hàng", "Tên khách hàng", "Mã hóa đơn", "Ngày lập", "Tổng tiền hóa đơn" });
		tb.clear();
	...
	public void MonAnCuaTungKhachHang_searchOnChange() {
		tb.clear();
		tb.setHeaders(new String[] { "Mã khách hàng", "Tên khách hàng", "Mã hóa đơn", "Ngày lập", "Mã món ăn",
				"Tên món ăn", "Số lượng " });
		...
		for (KhachHangDTO kh : qlkhBUS.getKhachHangDTO()) {
			...
			for (HoaDonDTO hd : qlhdBUS.search("Mã khách hàng", kh.getIDKhachHang(), mcd.getNgayTu(), mcd.getNgayDen(),
					-1, -1)) { // tương đối -> sai
				tb.addRow(new String[] { "", "", hd.getIDHoaDon(), String.valueOf(hd.getNgayLap()), "", "", "" });
				...
				for (ChiTietHoaDonDTO cthd : qlcthdBUS.search("Mã hóa đơn", hd.getIDHoaDon(), -1, -1, -1, -1)) { // tương
					...
					tongSoLuong += cthd.getSoLuong();
					tb.addRow(new String[] { "", "", "", "", cthd.getIDMonAn(),
							qlmaBUS.getMonAnDTO(cthd.getIDMonAn()).getTenMonAn(), String.valueOf(cthd.getSoLuong()) });
				}
			}
			tb.addRow(new String[] { "", "", "", mcd.getKhoangTG(), "", "Số phần món ăn",
					String.valueOf(tongSoLuong) + " phần " });
			tb.addRow(new String[] { "", "", "", "", "", "" });
...
class ThongKeNhaCungCap extends JPanel {
	...
	private void tongTienThanhToan() {
		tb.clear();
		tb.setHeaders(new String[] { "Mã nhà cung cấp", "Tên nhà cung cấp", "Mã hóa đơn nhập", "Ngày lập",
			...
			for (HoaDonNhapDTO hdn : qlhdnBUS.search("Tất cả", "", mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) {
				if (hdn.getIDNhaCungCap().equals(ncc.getIDNhaCungCap())) {
					...
					for (ChiTietHoaDonNhapDTO cthdn : qlcthdnBUS.search("Mã hóa đơn nhập", hdn.getIDHoaDonNhap())) {
						...
						tb.addRow(new String[] { "", "", "", "", cthdn.getIDNguyenLieu(),
