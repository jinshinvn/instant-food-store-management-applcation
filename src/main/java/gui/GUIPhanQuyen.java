...
public class GUIPhanQuyen extends GUIFormContent {
	...
	public String arrString_Quyen[] = { "Quản lý bán hàng", "Quản lý nhập hàng", "Quản lý món ăn",
			"Quản lý nguyên liệu", "Quản lý công thức", "Quản lý hóa đơn", "Quản lý hóa đơn nhập", "Quản lý khuyến mãi",
			"Quản lý khách hàng", "Quản lý nhân viên", "Quản lý nhà cung cấp", "Quản lý tài khoản",
				...
				if (a == JOptionPane.YES_OPTION) {
					if (checkTextThem(txt_PhanQuyen_Them[1].getText())) {
						PhanQuyenDTO DTO = new PhanQuyenDTO(txt_PhanQuyen_Them[0].getText(),
								txt_PhanQuyen_Them[1].getText(), layMoTaTuCheckBox(), "Hiện");
...
//        if (option == JOptionPane.YES_OPTION) {
		...
		PhanQuyenDTO DTO = new PhanQuyenDTO(txt_PhanQuyen_Sua[0].getText(), txt_PhanQuyen_Sua[1].getText(),
				layMoTaTuCheckBox());
...
//        }
	...
	protected void Sua_click(MouseEvent evt) {
		...
		if (i == -1) {
			...
			{
				if (PhanQuyenBUS.getMoTaQuyenTuMaQuyen(maQuyen).contains(arr_listmotaQuyen[j]))// so sánh xem mô tả
																								// quyền của thằng
//                    được chọn có mô tả những thứ được quản lý trong arr_listmotaQuyen hay không
	...
	protected void Xoa_click(MouseEvent evt) {
		...
		if (row == -1) {
			...
			if (option == JOptionPane.YES_OPTION) {
				if (checkXoaPhanQuyen(table_PhanQuyen.tb.getValueAt(row, 0).toString())) {
	...
	public void docDB() {
		...
		if (PhanQuyenBUS.dspq == null) {
			...
			try {
				...
				java.util.logging.Logger.getLogger(GUIPhanQuyen.class.getName()).log(java.util.logging.Level.SEVERE,
						null, ex);
			}
	...
	public void txtSearchOnChange() {
		...
		setDataToTable(Tool.searchPQ(search.getText(), cbSearch.getSelectedItem().toString()), table_PhanQuyen); // chưa
																													// sửa
																													// xong
																													// hỏi
																													// Nguyên
	...
	public boolean checkXoaPhanQuyen(String maQuyen)// kiểm tra xem mã quyền truyền vào có tồn tại trong mảng tài khoản
													// không nếu có thì
	// return true để thông báo không được xóa
	// return false để xóa được
	{
		for (TaiKhoanDTO taiKhoanDTO : TaiKhoanBUS.dstk) {
