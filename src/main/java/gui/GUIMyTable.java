...
public class GUIMyTable extends JPanel {
	...
	{
		addRow(new String[] { data.getIDMonAn(), data.getTenMonAn(), data.getDonViTinh(),
				String.valueOf((int) data.getDonGia()), data.getHinhAnh(), data.getLoai(),
				String.valueOf(data.getSoLuong()) });
	}

	public void addRow(NguyenLieuDTO data) // Add row nguyên liệu
	{
		addRow(new String[] { data.getIDNguyenLieu(), data.getTenNguyenLieu(), String.valueOf(data.getDonGia()),
				data.getHinhAnh(), data.getLoai(), data.getDonViTinh(), String.valueOf(data.getSoLuong()) });
	}

	public void addRow(KhachHangDTO data) {
		addRow(new String[] { data.getIDKhachHang(), data.getHoKhachHang(), data.getTenKhachHang(), data.getGmail(),
				data.getGioiTinh(), data.getSoDienThoai(), String.valueOf((int) data.getTongChiTieu())

		});
	}

	public void addRow(NhanVienDTO data) {
		addRow(new String[] { data.getIDNhanVien(), data.getHoNhanVien(), data.getTenNhanVien(), data.getGmail(),
				data.getGioiTinh(), data.getSoDienThoai(), data.getChucVu() });
	}

	public void addRow(HoaDonDTO data) {
		addRow(new String[] { data.getIDHoaDon(), data.getIDNhanVien(), data.getIDKhachHang(), data.getIDKhuyenMai(),
				String.valueOf(data.getNgayLap()), String.valueOf((int) data.getTienGiamGia()),
				String.valueOf((int) data.getTongTien()) });
	}

	public void addRow(HoaDonNhapDTO data) {
		addRow(new String[] { data.getIDHoaDonNhap(), data.getIDNhanVien(), data.getIDNhaCungCap(),
				String.valueOf(data.getNgayNhap()), String.valueOf((int) data.getTongTien()) });
	}

	public void addRow(NhaCungCapDTO data) {
		addRow(new String[] { data.getIDNhaCungCap(), data.getTenNhaCungCap(), data.getSoDienThoai(), data.getGmail(),
				data.getDiaChi() });
	}

	public void addRow(TaiKhoanDTO data) {
		addRow(new String[] { data.getTaiKhoan(), data.getIDNhanVien(), data.getIDPhanQuyen(), data.getMatKhau() });
	}

	public void addRow(PhanQuyenDTO data) {
		addRow(new String[] { data.getIDPhanQuyen(), data.getTenQuyen(), data.getMoTaQuyen() });
	}

	public void addRow(KhuyenMaiDTO data) {
		addRow(new String[] { data.getIDKhuyenMai(), data.getTenChuongTrinh(), String.valueOf((int) data.getTienGiam()),
				String.valueOf(data.getNgayBatDau()), String.valueOf(data.getNgayKetThuc()),
				data.getNoiDungGiamGia() });
	}

	public void addRow(CongThucDTO data, MonAnDTO monAnDTO) {
		addRow(new String[] { data.getIDCongThuc(), data.getIDMonAn(), monAnDTO.getTenMonAn(),
	...
	public void addRow(ChiTietHoaDonDTO data) {
		addRow(new String[] { data.getIDMonAn(), String.valueOf(data.getSoLuong()), String.valueOf(data.getDonGia()),
				String.valueOf((int) data.getThanhTien()) });
	}

	public void addRow(ChiTietHoaDonNhapDTO data) {
		addRow(new String[] { data.getIDNguyenLieu(), String.valueOf(data.getSoLuong()),
				String.valueOf(data.getGiaNhap()), String.valueOf((int) data.getThanhTien()) });
	}
	...
	public void setColumnsWidth(double[] percentages) // nhập vào mảng
	{
		...
		for (int i = 0; i < tb.getColumnModel().getColumnCount(); i++) {
			...
			column.setPreferredWidth((int) (getPreferredSize().width * (percentages[i] / total))); // đổ ra tỉ lệ phầm
																									// trăm
		}
	}
