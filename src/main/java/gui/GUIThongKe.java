...
class ThongKe_Hoang extends JPanel {
	...
	public ThongKe_Hoang() throws Exception {
		...
		tbThongKeHoaDon.setHeaders(new String[] { "Hóa đơn", "Tên nhân viên", "Tên khách hàng", "Tên món ăn",
		...
		tbThongKePhieuNhap.setHeaders(new String[] { "Hóa đơn nhập", "Tên nhân viên", "Tên nhà cung cấp",
...
//        plChonNgay.setVisible(false);
		...
		// event biến đổi panel tiềm kiếm theo tab bán hoặc nhập trong thống kê tổng
		// quát
...
//    Này là panel tổng trong thống kê tổng quát
	private void setDataToPanelTong() {
		...
		plThongKeSoLuong
				.add(getJPanelTong("NHÂN VIÊN", "nhanvien1-100.png", qlnvBUS.getNhanVienDTO().size(), Color.BLUE));
		plThongKeSoLuong
				.add(getJPanelTong("KHÁCH HÀNG", "khachhang1-100.png", qlkhBUS.getKhachHangDTO().size(), Color.BLUE));
		plThongKeSoLuong.add(
				getJPanelTong("NHÀ CUNG CẤP", "nhacungcap-100.png", qlnccBUS.getNhaCungCapDTO().size(), Color.BLUE));
	}
...
//   này là phần bán ra
	private void onChangeThongKeBanHang() throws Exception {
		...
		for (HoaDonDTO hd : qlhdBUS.search("Tất cả", "", mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) {
			...
			ArrayList<ChiTietHoaDonDTO> dscthd = qlcthdBUS.getALLChiTiet(hd.getIDHoaDon());// bổ sung thêm ở cthd bus

			if (dscthd.size() > 0) {
				NhanVienDTO nv = qlnvBUS.getNhanVienDTO(hd.getIDNhanVien()); // bổ sung thêm phần getNhanVienDTO,
																				// getKhachHangDTO giống ông Nhân
				...
				tbThongKeHoaDon.addRow(new String[] { hd.getIDHoaDon() + " (" + hd.getNgayLap().toString() + ")",
				...
				for (ChiTietHoaDonDTO cthd : dscthd) {
					...
					tbThongKeHoaDon.addRow(new String[] { "", "", "", ma.getTenMonAn() + " (" + ma.getIDMonAn() + ")",
							String.valueOf(cthd.getSoLuong()), PriceFormatter.format((float) cthd.getDonGia()),
			...
			tbThongKeHoaDon.addRow(new String[] { "", "", "", "", "", "Giảm giá",
					PriceFormatter.format((float) hd.getTienGiamGia()) });
		...
		tbKetQuaHoaDon.addRow(
				new String[] { tongSLHoaDon + " hóa đơn", dsnv.size() + " nhân viên", dskh.size() + " khách hàng",
						dsma.size() + " món ăn", tongSLMonAn + " phần ", "", PriceFormatter.format(tongTatCaTien) });
	}
...
//                // bỏ qua lần lặp for này nếu nhân viên hoặc nha cung cap ko thỏa bộ lọc
				...
				tbThongKePhieuNhap
						.addRow(new String[] { hdn.getIDHoaDonNhap() + " (" + hdn.getNgayNhap().toString() + ")",
								nv.getTenNhanVien() + " (" + nv.getIDNhanVien() + ")",
				...
				for (ChiTietHoaDonNhapDTO dsctpn : cthdn) {
					...
					tbThongKePhieuNhap.addRow(
							new String[] { "", "", "", nl.getTenNguyenLieu() + " (" + nl.getIDNguyenLieu() + ")",
									String.valueOf(dsctpn.getSoLuong()), PriceFormatter.format(dsctpn.getGiaNhap()),
			...
			tbThongKePhieuNhap
					.addRow(new String[] { "", "", "", "", "", "Tổng cộng", PriceFormatter.format(tongTien) });
		...
		tbKetQuaPhieuNhap.addRow(new String[] { tongSLPhieuNhap + " hóa đơn nhập", dsnv.size() + " nhân viên",
				dsncc.size() + " nhà cung cấp", dsnl.size() + " loại", tongSLSanPham + " phần", "",
